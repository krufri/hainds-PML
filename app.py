...
def main():
    ...
    while True:
        ...
        if len(faces) > 0 and execute_skin_color_detection == True:
            execute_skin_color_detection = False
            face_points = faces[0]
            ...
            thread.start()
        elif len(faces) == 0 and execute_skin_color_detection == True:
            execute_skin_color_detection = False
        ...
        if results.multi_hand_landmarks is not None:
            ...
            if len(results.multi_handedness) == 2:
                ...
                absolut_two_hands_landmark_list = list()
                ...
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    absolut_two_hands_landmark_list.extend(temp_landmark_list)


                    #index position
                    position_index = landmark_list[8]
                ...
                distance_index = sqrt((position_index_list[1][0] - position_index_list[0][0])**2) + ((position_index_list[1][1] - position_index_list[0][1])**2)
                # Write to the dataset file
                logging_csv(number, mode, two_hands_landmark_list, pre_processed_point_history_list, distance_index)


                 # Hand sign classification
                hand_sign_id = handgesture_classifier(two_hands_landmark_list)
                two_hands_landmark_list.append(distance_index)
                print(hand_sign_id)
                ...
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    pre_processed_point_history_list = pre_process_point_history(
                        debug_image, point_history)
                    one_hand_landmark_list = list()
                    one_hand_landmark_list = [0] * 42
                    one_hand_landmark_list.extend(pre_processed_landmark_list)
                    # Write to the dataset file
                    logging_csv(number, mode, one_hand_landmark_list, pre_processed_point_history_list, 0)


                    # Hand sign classification
                    hand_sign_id = handgesture_classifier(one_hand_landmark_list)
                    #point_history.append([0, 0])
                    #one_hand_landmark_list.append(0)

                    if hand_sign_id == 3:  # Wave or Raising
                        point_history.append(landmark_list[12])
                    elif hand_sign_id == 19:
                        point_history.append(landmark_list[12])
                    #elif hand_sign_id == 6:  # Point gesture
...
def logging_csv(number, mode, landmark_list, point_history_list, distance):
    if mode == 0:
        pass
    if mode == 1 and (0 <= number <= 29):
        csv_path = 'model/handgesture_classifier/handgesture_landmarks.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list, distance])
    if mode == 2 and (0 <= number <= 9):
        csv_path = 'model/point_history_classifier/point_history.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *point_history_list, distance])
    return
...
def draw_info_text(image, brect, handednessInfo, hand_sign_text, finger_gesture_text, hand_sign_id, finger_gesture_id):
    cv.rectangle(image, (brect[0], brect[1]), (brect[2], brect[1] - 22),
                 (0, 0, 0), -1)

    info_text = handednessInfo
    if hand_sign_text != "":
        info_text = info_text + ':' + hand_sign_text
    cv.putText(image, info_text, (brect[0] + 5, brect[1] - 4),
               cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv.LINE_AA)

    if hand_sign_id == 3 and finger_gesture_id == 1 or hand_sign_id == 3 and finger_gesture_id == 2 or hand_sign_id == 18 and finger_gesture_id == 3 :
...
def draw_info(image, fps, mode, number):
    ...
    cv.putText(image, "Press Space to detect skin color", (10, 70), cv.FONT_HERSHEY_SIMPLEX,
               1.0, (0, 0, 0), 4, cv.LINE_AA)
    cv.putText(image, "Press Space to detect skin color", (10, 70), cv.FONT_HERSHEY_SIMPLEX,
               1.0, (255, 255, 255), 2, cv.LINE_AA)
...
def classify_skin_color(model, face_parser, image):
    print("classify_skin_color called")
    class_names = ['dark', 'light', 'medium', 'veryDark', 'veryLight']
    # use edited_image when correcting gamma
